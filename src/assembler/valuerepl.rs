                ...
                match arg[2].as_str() {
                    "PAGE" => {
                        ...
                        if section_idx.is_none() {
                            ...
                            let error = format!("Section named {} called in line {} doesn't exist. This might be a bug, if you're sure it is, file a report.", arg0_page, real_line_number).red().to_string();
                            eprintln!("{}", error);
                            exit(105);
                        }
